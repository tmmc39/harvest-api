...
const getApy = async (
  ...
  try {
    const response = await cachedAxios.get('https://api.curve.fi/api/getFactoGaugesCrvRewards/arbitrum')
    const apyResults = get(response, `data.data.sideChainGaugesApys`, []);
    const result = apyResults.find(value => value.address.toLowerCase() === gaugeAddress.toLowerCase());
